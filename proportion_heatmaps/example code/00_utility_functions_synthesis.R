...
#########################
# This function takes species occurrence points and twgd region polygons and tallies up 
#   species richness for each twgd polygon based on those occurrence points
...
organize.bubble.plot2 <- function(points, twgd_data, colname = "n_points", file = "list_result1.Rsave") {
  ...
  colnames(results)[colnames(results) == "n_points"] <- colname
  save(list_result1, file=file)
  return(results)
}
...
organize.bubble.plot <- function(trait_table, reference_table, all_vars, twgd_data) {
  tmp_reference_table <- subset(reference_table, reference_table$wcvp_loadNamespacname %in% unique(trait_table$species))
  ...
  for(i in 1:length(focal_areas)) {
    ...
    area_plus_buffer <- twgd_data[which(as.character(twgd_data$LEVEL3_COD) %in% one_area),]
    if(nrow(area_plus_buffer)>0) {
      centroids <- sf::st_centroid(area_plus_buffer, byid=TRUE)
