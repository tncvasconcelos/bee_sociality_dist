...
merged_climatic_vars <- climatic_list[[1]] # initializes merged climate dataframe using first climate variable in list

for (i in 2:length(climatic_list)) { # adds each remaining climate variable one by one (series of merges by species name)
  merged_climatic_vars <- merge(merged_climatic_vars, climatic_list[[i]], by = "species") 
}
...
#-------------------------------------------------------------------------------
merged_traits$mean_bio_1 <- log(merged_traits$mean_bio_1 + 273) # convert °C to Kelvin for temperature
...
#     Estimates one optimum for all social species and one for all solitary species, 
